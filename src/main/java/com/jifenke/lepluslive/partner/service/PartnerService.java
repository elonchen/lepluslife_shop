...
public class PartnerService {
  ...
  /**
   * 合伙人的锁定会员信息  17/05/11 1.合伙人锁定会员； 2.合伙人锁定会员数； 3.今日锁定会员数。
   */
  @Transactional(propagation = Propagation.REQUIRED, readOnly = true)
  public Map findUserByPartner(Partner partner, Integer currPage) {
    ...
    List<LeJiaUser>
        bindUsers =
        leJiaUserRepository.findByBindPartnerAndPage(partner.getId(), currPage);
  ...
  /**
   * 合伙人佣金记录  17/05/12 1.线下佣金记录 2.线上佣金记录 3.总佣金
   */
  @Transactional(propagation = Propagation.REQUIRED, readOnly = true)
  public Map findPartnerCommisssion(Partner partner, Integer currPage) {
    Map result = new HashMap();
    //  佣金记录
    List<PartnerWalletOnlineLog>
        onlineLogs =
        partnerWalletOnlineLogRepository.findByPartnerIdAndPage(partner.getId(), currPage);
    List<PartnerWalletLog>
        offLineLogs =
        partnerWalletLogRepository.findByPartnerIdAndPage(partner.getId(), currPage);
    //  佣金总计
    ...
    Long
        totalCommission =
        (sumOnLine == null ? 0 : sumOnLine) + (sumOffLine == null ? 0 : sumOffLine);
  ...
  /**
   * 合伙人的好店信息  17/05/12 1.门店名称 2.每日佣金 3.锁定会员数
   */
